...
class layers_conv:
    ...
    def __init__(self, k, data, filters, time, dt, n_points, ring_option, C, p, **kwargs):
        self.k = k
        self.data = data
        self.time = time
        self.dt = dt
        ...
        self.p_in = p
        self.W = np.zeros(( filters, k, k))
        self.dW = np.zeros(( filters, k, k, int(time/dt)))
    ...
    def conv_op(self, vis):
        ...
        for i in range(self.filter_size):
            for j in range(self.filter_size):
                ...
                for ch in range(1): # input channels
                    ...
                    #stimu, self.W[i,j,:,:] = conv_ring(self.data[0,i:i+self.k,j:j+self.k,:],self.k, option=self.ring_option)
                    p = self.p_in[i:i+self.k,j:j+self.k,:]
                    p = np.resize(p, (self.k*self.k, 500))
                    
                    stimuli = self.data[0,i:i+self.k,j:j+self.k,:]
                    stimuli = np.resize(stimuli, (self.k,self.k,500))
                    
                    #print(np.shape(self.time), np.shape(stimu), np.shape(self.dt) )
                    temp1, m, dW_temp, p_test, W = LIF.multiple_neurons(time=self.time, stimuli=stimuli, n_neurons=int(self.k*self.k), dt=self.dt, stdp=True, p=p)
                    # index=np.where(dW_temp!=0)
                    # if np.size(index) !=0:
                    #     print('OK !')
                    #temp2 = temp1 + temp2
   
                    # indexes=np.where(dW_temp>0.001)
                    # if np.size(indexes)!=0:
                    #     for aux in range(9):
                            
                    #         plt.plot(dW_temp[aux,:])
                    #         plt.title("dW")
                    #         plt.show()
                ...
                intensity[i,j,:]=temp1
                #print(np.shape(dW_temp))
                #dW_val = np.sum(dW_temp, axis=-1)
                dW_temp = np.resize(dW_temp, (self.k, self.k, np.shape(dW_temp)[-1]))
                index=np.where(dW_temp!=0)
                if np.size(index) !=0:
                     print('OK0 !')
                     print(np.shape(dW_temp))
        ...
        # print('D1', np.shape(intensity) )
        # print('D1', np.shape(p_test) )
        # print('D1', np.shape(dW_val) )
        # print('D1', np.shape(W) )
        return intensity, p_test, dW_temp, W
...
# v1, p1, m1, dW1 = LIF.single_neuron(stimuli=I_, dt=dt, time=time, stdp=False, p=p)
    ...
    def conv2D(self, vis):
        ...
        for i in range(0,self.filters):
            L_map[i,:,:,:], p_test, debug, self.W[i,:,:]=self.conv_op(vis=vis)
            index=np.where(debug!=0)
            if np.size(index) !=0:
                print('OK1111 !')
