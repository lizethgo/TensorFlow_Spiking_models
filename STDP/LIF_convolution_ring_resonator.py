...
step_length = 2 ## change accordingly
...
#### transforming to input spikes
test=np.zeros([np.shape(x_train)[1], np.shape(x_train)[1],int(N_points)])
stdp_p=np.zeros([np.shape(x_train)[1], np.shape(x_train)[1],int(N_points)])
for i in range(np.shape(x_train)[1]):
    for j in range(np.shape(x_train)[1]):
        p = 0
        LIF = LIF_RK(C=0.00005)
        test[i,j,:] = LIF.single_neuron(stimuli=d[i,j,:], dt=dt, time=time, stdp=False, p=p)[0]
        stdp_p[i,j,:] = LIF.single_neuron(stimuli=d[i,j,:], dt=dt, time=time, stdp=False, p=p)[1]

        
test = np.reshape(test, (int(np.shape(x_train)[1]*np.shape(x_train)[1]),N_points)) 
indexes=np.where(test>0.5)
fig = plt.figure(figsize=(6, 4))
plt.scatter(indexes[1][:], indexes[0][:], marker='|', s=15, color='red')
plt.show()   


test = np.reshape(test, (1,np.shape(d)[0], np.shape(d)[1], np.shape(d)[-1]))
#### Sequential model
Layer_1 = layers_conv(k=3,filters=10, data=test, dt=dt, n_points=N_points, time=time, ring_option=1, p=stdp_p, C=0.00005)
x1 = Layer_1.conv2D(vis=True)
