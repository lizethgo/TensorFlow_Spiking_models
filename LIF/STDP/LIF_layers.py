...
class layers_conv:
    ...
    def __init__(self, k, data, filters, time, dt, n_points, ring_option, C, **kwargs):
        ...
        self.C=C
        self.dW = np.zeros((k,k,filters)) # gradients of weights
        ...
        for i in range(self.filter_size):
            for j in range(self.filter_size):
                ...
                for ch in range(1): # input channels
                    ...
                    temp1, p, m, dW = LIF.single_neuron(time=self.time, stimuli=stimu, dt=self.dt, stdp=False, p=p_test)
                    temp2 = temp1 + temp2
